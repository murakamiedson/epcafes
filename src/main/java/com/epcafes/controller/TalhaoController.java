package com.epcafes.controller;

import java.util.List;
import java.util.Optional;
import java.util.stream.Collectors;
import java.util.stream.IntStream;

import org.springframework.beans.factory.annotation.Autowired;
import org.springframework.security.core.Authentication;
import org.springframework.security.core.context.SecurityContextHolder;
import org.springframework.stereotype.Controller;
import org.springframework.ui.Model;
import org.springframework.validation.BindingResult;
import org.springframework.web.bind.annotation.GetMapping;
import org.springframework.web.bind.annotation.PathVariable;
import org.springframework.web.bind.annotation.PostMapping;
import org.springframework.web.bind.annotation.RequestParam;

import com.epcafes.exception.BusinessException;
import com.epcafes.model.Talhao;
import com.epcafes.model.Usuario;
import com.epcafes.service.TalhaoService;

import lombok.extern.log4j.Log4j2;

@Log4j2
@Controller
public class TalhaoController {
	
	@Autowired
	private TalhaoService talhaoService;
	
	@GetMapping({ "/CadastroTalhao", "/CadastroTalhao/edit/{id}" })
    public String listarTalhao(Talhao talhao, Model model,
            @RequestParam("page") Optional<Integer> page,
            @RequestParam("size") Optional<Integer> size,
            @RequestParam("qtdPorPagina") Optional<Integer> qtdPorPagina,
            @PathVariable(name = "id") Optional<Long> id) {
        int currPage = page.orElse(1);
        int currSize = size.orElse(5);
        int pageSize = size.orElse(5);
        int qtdPorPaginaInt = qtdPorPagina.orElse(5);

        List<Talhao> talhoes = talhaoService.findPaginated(currPage, pageSize);
        int qtdPaginas = (int) Math.ceil(talhaoService.findAll().size() / (double) pageSize);
        List<Integer> pageNumbers = IntStream.rangeClosed(1, qtdPaginas).boxed().collect(Collectors.toList());
        List<Integer> qtdPorPaginaList = List.of(1, 2, 5, 10, 15, 20, 25);

        model.addAttribute("talhoes", talhoes);
        model.addAttribute("pageNumbers", pageNumbers);
        model.addAttribute("qtdPaginas", qtdPaginas);
        model.addAttribute("currPage", currPage);
        model.addAttribute("qtdPorPagina", qtdPorPaginaInt);
        model.addAttribute("qtdPorPaginaList", qtdPorPaginaList);
        model.addAttribute("size", currSize);

        if (id.isPresent()) {
            talhao = talhaoService.findById(id.get());
            model.addAttribute("talhao", talhao);
        }

        return "restricted/cadastro/CadastroTalhao";
    }

    @PostMapping({ "/CadastroTalhao", "/CadastroTalhao/edit/CadastroTalhao" })
    public String novo(Talhao talhao, BindingResult result) throws BusinessException {
    	Authentication auth = SecurityContextHolder.getContext().getAuthentication();
        Usuario user = (Usuario) auth.getPrincipal();
        
        if (result.hasErrors()) {
            log.info("Erro: " + result.toString());
            return "restricted/cadastro/CadastroTalhao";
        }

        if (talhao.getId() != null) {
        	Talhao existingTalhao = talhaoService.findById(talhao.getId());
            log.info("Atualizando Talhao existente: " + existingTalhao.toString());

            existingTalhao.setNome(talhao.getNome());
            existingTalhao.setArea(talhao.getArea());
            talhaoService.save(existingTalhao);
        } else {
            log.info("Salvando Novo Talhao: " + talhao.toString());
            talhao.setTenant_id(user.getTenant().getId());
            talhao.setPropriedade(user.getPropriedade());
            talhaoService.save(talhao);
        }
        
        return "redirect:/CadastroTalhao";
    }

    @GetMapping("/CadastroTalhao/delete/{id}")
    public String delete(@PathVariable(name = "id") Long id) throws BusinessException{

        Talhao talhao = talhaoService.findById(id);
        talhaoService.delete(talhao);

        return "redirect:/CadastroTalhao";
    }

    /* Modal de Cadastro de Talhao */
    @GetMapping("/CadastroTalhao/modal")
    public String modalTalhao(Model model, Optional<Long> id) {
      
        Talhao talhao;
        if(id.isPresent()){
            talhao = talhaoService.findById(id.get());
        }else{
            talhao = new Talhao();
        }
        model.addAttribute("talhao", talhao);

        return "restricted/cadastro/ModalTalhao";
    }
}